...
impl Metadata {
    ...
    pub fn servings(&self) -> Option<u32> {
        self.get(StdKey::Servings)
            .and_then(CooklangValueExt::as_u32)
    }
...
fn value_as_servings(val: &serde_yaml::Value) -> Result<u32, MetadataError> {
    let servings: u32 = if let Some(n) = val.as_u32() {
        n
    } else {
        return Err(MetadataError::expect_type(MetaType::Number, val));
    };
...
pub(crate) fn check_std_entry(
    ...
    match key {
        StdKey::Servings => {
            return Ok(Some(crate::scale::Servings(Some(value_as_servings(value)?))));
            // return value.as_u32().ok_or(MetadataError::expect_type(MetaType::Number, value))
        }
        StdKey::Tags => {
            value_as_tags(value)?;
        }
